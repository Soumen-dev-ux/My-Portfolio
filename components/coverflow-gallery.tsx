...
// Mock Framer's static renderer hook for client-side execution
...
/**
 * Smooth 3D Slideshow / Coverflow Gallery
 *
...
export default function Smooth3DSlideshow(props: Smooth3DSlideshowProps) {
    ...
    const handleCardClick = useCallback(
        (i: number) => {
            ...
            lock()
            if (i === active) {
                // If it's already active, invoke custom click handler (e.g. navigation)
                if (onCardClick) {
                    onCardClick(i)
                } else if (autoplay) {
                    setActive((a) => (a + 1) % n)
        ...
        [isStatic, autoplay, n, lock, active, onCardClick]
    )
