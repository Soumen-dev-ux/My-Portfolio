...
export default function VaporizeTextCycle(props: VaporizeTextCycleProps = {}) {
    ...
    const timing = useMemo(
        () => ({
            ...
            inDelay: Math.max(0, delayOf(appear.transition, 0)),
    ...
    // Find the tallest text in character length to use as a spacer.
    // This gives the parent container a stable, resize-friendly height.
    const tallestText = useMemo(() => {
        return texts.reduce(
            (tallest, current) => (current.length > tallest.length ? current : tallest),
            ""
        );
    }, [texts]);

    // Spacing style for dynamic layout height (rendered as transparent text)
    const invisibleTextStyle = useMemo<React.CSSProperties>(
        () => ({
            fontFamily: font.fontFamily,
            fontWeight: font.fontWeight,
            fontSize: font.fontSize,
            lineHeight: font.lineHeight ?? 1.5,
            textAlign: (font.textAlign || alignment) as any,
            color: "transparent",
            userSelect: "none",
            pointerEvents: "none",
            margin: 0,
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            if (phaseRef.current === "out") {
                ...
                const elapsed = phaseTimeRef.current - t.inDelay;
                if (elapsed < 0) {
                    ctx.clearRect(0, 0, canvas.width, canvas.height);
                    frameId = requestAnimationFrame(animate);
                    return;
                }
                const p = Math.min(1, elapsed / t.inDuration);
                const e = t.inEase(p);
                if (
                    startsArrRef.current !== particles ||
                ...
                if (t.inMode === "particle") {
                    ...
                    for (let i = 0; i < particles.length; i++) {
                        ...
                        // Magic glowing particles (mix original color with orange/yellow magic dust)
                        if (particle.targetR === undefined) {
                            const isYellow = Math.random() > 0.6;
                            particle.targetR = isYellow ? 250 : 249;
                            particle.targetG = isYellow ? 204 : 115;
                            particle.targetB = isYellow ? 21 : 22;
                        }

                        const colorMix = 1 - local;
                        particle.r = Math.round(
                            particle.originalR + (particle.targetR - particle.originalR) * colorMix
                        );
                        particle.g = Math.round(
                            particle.originalG + (particle.targetG - particle.originalG) * colorMix
                        );
                        particle.b = Math.round(
                            particle.originalB + (particle.targetB - particle.originalB) * colorMix
                        );
                ...
                if (live.textCount > 1 && phaseTimeRef.current >= t.hold) {
                    resetParticles(particles);
    ...
    return (
        <div ref={wrapperRef} style={wrapperStyle}>
            ...
            <p style={invisibleTextStyle}>{tallestText}</p>
...
const renderCanvas = ({
    ...
    // Use wrapped text width logic instead of single-line width
    const boxW = width * globalDpr;
    const widestWrapped = (framerProps.texts ?? [currentText]).reduce(
        (maxW: number, t: string) => {
            const lines = wrapText(ctx, t, boxW);
            const maxLineW = lines.reduce((maxL, l) => Math.max(maxL, ctx.measureText(l).width), 0);
            return Math.max(maxW, maxLineW);
        },
        0
    );

    const widest = widestWrapped;
...
const createParticles = (
    ...
    const fontSize = parseInt(font.match(/\d+px/)?.[0] || "18");
    ...
    // Draw lines centered vertically around textY
    ...
    const y0 = Math.max(0, Math.floor(textY - totalHeight / 2 - pad));
    const x1 = Math.min(canvas.width, Math.ceil(textLeft + textWidth + pad));
    const y1 = Math.min(canvas.height, Math.ceil(textY + totalHeight / 2 + pad));
        ...
        for (let x = 0; x < boxW; x += sampleRate) {
            ...
            if (alpha > 0) {
                ...
                particles.push({
                    ...
                    g: data[index + 1],
                    b: data[index + 2],
                    originalR: data[index],
                    originalG: data[index + 1],
                    originalB: data[index + 2],
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        if (shouldVaporize) {
            if (particle.speed === 0) {
                ...
                // Doctor strange portal magical color assignment
                const isYellow = Math.random() > 0.6;
                particle.targetR = isYellow ? 250 : 249;
                particle.targetG = isYellow ? 204 : 115;
                particle.targetB = isYellow ? 21 : 22;
            ...
            // Interpolate colors towards orange/yellow magic spark as it vaporizes
            const colorMix = local;
            particle.r = Math.round(
                particle.originalR + (particle.targetR - particle.originalR) * colorMix
            );
            particle.g = Math.round(
